...
/// 에러는 호출자에게 전파됩니다.
pub fn list_entries<P: AsRef<Path>>(path: P, i: i32) -> std::io::Result<Vec<(PathBuf, EntryType, i32)>> {
    let p = path.as_ref();

    if is_file(p)? {
        return Ok(vec![(p.to_path_buf(), F, i)]);
    }
    ...
    if is_directory(p)? {
        ...
        for entry_res in read_dir(p)? {
            ...
            if is_file(entry.path())? {
                ...
                // 파일인 경우, 항목에 추가
                items.push((entry.path(), F, i));
            } else if is_directory(entry.path())? {
                items.push((entry.path(), D, i));

                // 디렉터리인 경우, 재귀적으로 항목들을 추가
                let sub_items = list_entries(entry.path(), i + 1)?;
